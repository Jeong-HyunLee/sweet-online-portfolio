  ...
  <section id="members" className="py-20">
    <div className="container max-w-5xl">
      ...
      <div className="mt-10">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {currentMembers.map((m) => (
            <div key={m.name} className="rounded-md border bg-card p-5">
              ...
              <p className="mt-2 text-xs text-muted-foreground">{m.topic}</p>
              <p className="mt-1 text-[10px] font-medium text-accent">{m.period}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-14">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {alumni.map((m) => (
            <div key={m.name} className="rounded-md border bg-card/60 p-5">
              <p className="font-semibold text-foreground text-sm">{m.name}</p>
              <p className="mt-1 text-xs font-medium text-accent">{m.role}</p>
              <p className="mt-2 text-xs text-muted-foreground">{m.topic}</p>
              <p className="mt-1 text-[10px] text-muted-foreground/60">{m.period}</p>
            </div>
          ))}
        </div>
      </div>
